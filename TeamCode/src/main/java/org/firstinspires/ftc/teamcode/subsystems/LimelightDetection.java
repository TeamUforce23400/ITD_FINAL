...
public class LimelightDetection {
     ...
     *  6) If resultExists, writes out corner coordinates
     */
    public void getResult() {
        // 1) Fetch latest Limelight result
        LLResult result = limelight.getLatestResult();
        if (result == null) {
            telemetry.addData("Limelight", "getLatestResult() == null");
            telemetry.update();
            return;
        }
        if (!result.isValid()) {
            telemetry.addData("Limelight", "result.isValid() == false");
            telemetry.update();
            return;
        }
        ...
        // 5) If found, extract that blob’s corners into points[]
        if (resultExists) {
            ...
            telemetry.addData("Limelight", "No suitable target passed filters");
        }
